...
export function CartPage({cartLines = []}: CartPageProps) {
  const [open, setOpen] = useState(false);
  const {items, updateQuantity, removeItem} = useCartStore();

  // Use Zustand store items if cartLines is empty
  const products = cartLines.length > 0 ? cartLines : items;

  const subtotal = products.reduce((sum, product) => {
    if ('quantity' in product && 'price' in product) {
      return sum + product.quantity * product.price;
    }
    if ('quantity' in product && 'cost' in product) {
      return (
        sum + product.quantity * parseFloat(product.cost.totalAmount.amount)
      );
    }
    return sum;
  }, 0);
            ...
            {products.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-600 mb-6">
                  Börja handla genom att bläddra bland våra lager och produkter
                </p>
                <a
                  href="/collections"
                ...
                {products.map((product, productIdx) => (
                  <li key={product.id} className="flex py-6 sm:py-10">
                    <div className="shrink-0">
                      {'merchandise' in product && product.merchandise.image ? (
                        <ShopifyImage
                          data={product.merchandise.image}
                        ...
                        <div className="size-24 rounded-md bg-gray-200 flex items-center justify-center sm:size-48">
                        ...
                        <div>
                          <div className="flex justify-between">
                            <h3 className="text-sm">
                              <a
                                href={`/products/${'merchandise' in product ? product.merchandise.product.handle : '#'}`}
                                className="font-medium text-gray-700 hover:text-gray-800"
                              >
                                {'merchandise' in product
                          ...
                          <div className="mt-1 flex text-sm">
                            {'merchandise' in product &&
                              product.merchandise.selectedOptions?.map(
                                (option, index) => (
                                  <p
                                    key={option.name}
                                    className={
                          ...
                          <p className="mt-1 text-sm font-medium text-gray-900">
                            {'cost' in product ? (
                              <Money data={product.cost.totalAmount} />
                            ) : (
                              `${product.price} SEK`
                            )}
                          </p>
                        ...
                        <div className="mt-4 sm:mt-0 sm:pr-9">
                          <div className="inline-grid w-full max-w-16 grid-cols-1">
                            <select
                              ...
                              aria-label={`Kvantitet, ${'merchandise' in product ? product.merchandise.product.title : product.title}`}
            ...
            >
              ...
              <dl className="mt-6 space-y-4">
                ...
                <div className="flex items-center justify-between border-t border-gray-200 pt-4">
                  ...
                  <dd className="text-sm font-medium text-gray-900">
                    {shipping === 0 ? 'Gratis' : `${shipping} SEK`}
                  </dd>
                </div>
                <div className="flex items-center justify-between border-t border-gray-200 pt-4">
              ...
              <div className="mt-6">
                <button
                  type="submit"
                  className="w-full rounded-md border border-transparent bg-indigo-600 px-4 py-3 text-base font-medium text-white shadow-sm hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-gray-50 focus:outline-none transition-colors"
                >
                  Till kassan
                </button>
              </div>
